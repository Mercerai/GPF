...
def train(args, dataloader, test_loader, model, optimizer, global_step = 0):
    # wandb.init(
    #     # set the wandb project where this run will be logged
    #     project="PointAug-NeRF-on-DTU",
    #     name=args.expname,
    #     # entity="pointai",
    #     config=args
    # )
...
#
    ...
    #list[n]:(B(1),3,H,W) #(1,N,3) (1,n,3,3) (1,n,4,4) n = n_views
            ...
            if args.sample_strategy.train == "surface":
                ...
                pts_info = rend_utils.log_sampling(args, rays_o, rays_d, dpts[:, 0], selected_inds, depth_range, noise=True)
            else:
            ...
            # wandb.log({"total loss":loss.item(), "rgb loss":loss_dict["rgb loss"].item(),
            #            "mask loss":loss_dict["mask loss"].item(), "depth loss":loss_dict["depth loss"].item()})
    ...
    with torch.no_grad():
        if specific_idx is not None:
            ...
            if args.sample_strategy.test == "surface":
                ...
                pts_info = rend_utils.log_sampling(args, rays_o, rays_d, dpts[:, 0], selected_inds, depth_range, noise=False)
            else:
                raise NotImplementedError
...
def main(args):
    ...
    # params = []
    # for key, value in renderer.named_parameters():
    #     if not value.requires_grad:
    #         continue
    #     params += [{"params": [value], "lr": args.training.lr, "weight_decay": args.training.weight_decay, "eps": args.eps}]
    # optimizer = torch.optim.Adam(params, args.training.lr, args.training.weight_decay)
    ...
    train_dataset = DTUDataset(
        ...
        scan_list=args.data.scan_list,
        num_light_idx=7,
        robust_train=args.training.robust_train,
    ...
    train_loader = Data.DataLoader(dataset=train_dataset, batch_size=1, num_workers=args.data.num_workers, shuffle=args.training.shuffle)
    ...
    train(args, train_loader, test_loader, renderer, optimizer, global_step)
