...
def read_ply(filename, downsample=-1, downsample_method="uniform"):
    # ply_data = PlyData.read(filename)
    # x = np.asarray(ply_data["vertex"].data['x'])
    # y = np.asarray(ply_data["vertex"].data['y'])
    # z = np.asarray(ply_data["vertex"].data['z'])
    pcd = o3d.io.read_point_cloud(filename)
    # r = np.asarray(ply_data.elements[0].data['red'])
    # g = np.asarray(ply_data.elements[0].data['green'])
    # b = np.asarray(ply_data.elements[0].data['blue'])
    # xyz = np.stack([x, y, z], axis=1)
    # print("before sample ", pcd)
    if downsample > 0:
        if downsample_method == "voxel":
            ...
            raise NotImplementedError("%s is not implemented for sampling points" % downsample_method)
    # print("after sample ", pcd)

    points = np.asarray(pcd.points)

    # points = points - pcd.get_center()
    # points = points * 1.0295 + pcd.get_center()
...
def glob_txts(path):
    txts = []
    for ext in ["*.txt"]:
        txts.extend(glob.glob(os.path.join(path, ext)))
    return txts

def glob_pfms(path):
    pfms = []
    for ext in ["*.pfm"]:
        pfms.extend(glob.glob(os.path.join(path, ext)))
    return pfms